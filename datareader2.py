...
class YcbineoatReader:
    def __init__(self, video_dir=None, downscale=1, shorter_side=None, zfar=np.inf):
        self.video_dir = video_dir
        self.downscale = downscale
        self.zfar = zfar
        self.color_files = sorted(glob.glob(f"{self.video_dir}/rgb/*.png")) if video_dir else []
        self.K = np.loadtxt(f'{video_dir}/cam_K.txt').reshape(3, 3) if video_dir else None
        self.id_strs = []
        ...
        if shorter_side is not None and self.H and self.W:
        ...
        self.gt_pose_files = sorted(glob.glob(f'{self.video_dir}/annotated_poses/*')) if video_dir else []
